...
function EditarEvento({ evento, editar, subirImagen, onCerrar }: Props) {
  ...
  const manejarSubmit = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setSubiendo(true)
        let imagen_url: string | undefined = preview || undefined
        if (imagen) {
          const url = await subirImagen(imagen, evento.id)
